...
class SoftgymCloth3d : public Scene
{
public:
    float cam_x;
    float cam_y;
    ...
    SoftgymCloth3d(const char *name) : Scene(name) {}
    ...
    char *make_cloth3d_path(char *full_path, std::string path)
    {
        strcpy(full_path, getenv("CLOTH3D_PATH"));
        strcat(full_path, path.c_str());
        return full_path;
    }
    ...
    void createCloth3d(const char *filename, string cloth_prefix, Vec3 lower, float radius, float rotation, Vec3 velocity, int phase, float invMass, float stiffness, float scale)
    // Scale the cloth until the average edge length is the same as particle radius
    {
        ...
        // vertex particle
        // char map_path[200];
        // ofstream mapfile(make_cloth3d_path(map_path, "/" + cloth_prefix + "/" + +"_map.txt"));
        ...
        // ofstream edgemapfile(make_path(edgemap_path, "/cloth3d/" + cloth_prefix + "_edgemap_id.txt"));
    ...
    void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
    {
        ...
        char *obj_path = make_cloth3d_path(cloth_path, "/" + cloth_prefix + "/" + cloth_index + ".obj");
        cout << "creating " << cloth_prefix << " from " << obj_path << endl;
        createCloth3d(obj_path, cloth_prefix, Vec3(initX, initY, initZ), radius, rot, Vec3(velX, velY, velZ), phase, 1 / mass, stiff, scale);

        g_numSubsteps = 4;
        g_params.numIterations = 30;
        g_params.dynamicFriction = 0.75f; // 0.75
        g_params.staticFriction = 1.0f;   // 0.0

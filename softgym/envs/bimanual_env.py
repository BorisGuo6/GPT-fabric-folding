...
class BimanualEnv(FlexEnv):
    ...
    def get_rgbd(self, cloth_only=False):
        ...
        rgb = rgb.reshape((720, 720, 4))[::-1, :, :] # reverse the height dimension
        rgb = rgb[:, :, :3]
        depth = np.array(depth).reshape(self.camera_params['default_camera']['height'], self.camera_params['default_camera']['width'])
        depth = depth.reshape(720,720)[::-1] # reverse the height dimension
        rgb = cv2.resize(rgb, (image_dim, image_dim), interpolation=cv2.INTER_LINEAR)
        depth = cv2.resize(depth, (image_dim, image_dim), interpolation=cv2.INTER_LINEAR)

    ...
    def _apply_picker_boundary(self, picker_pos):
        ...
        for i in range(3):
            clipped_picker_pos[i] = np.clip(
                picker_pos[i], self.picker_low[i] + self.picker_radius, self.picker_high[i] - self.picker_radius
            )
            ...
            if pick_flag[i]:
                if (
                    ...
                    dists = scipy.spatial.distance.cdist(
                        picker_pos[i].reshape((-1, 3)), particle_pos[:, :3].reshape((-1, 3))
                    )
...
class PickerPickPlace(Picker):
    def __init__(self, num_picker, env=None, picker_low=None, picker_high=None, **kwargs):
        super().__init__(num_picker=num_picker, picker_low=picker_low, picker_high=picker_high, **kwargs)
...
class PickerQPG(PickerPickPlace):
    def __init__(self, image_size, cam_pos, cam_angle, full=True, **kwargs):
        ...
        self.action_space = Box(
            np.array([-1.0, -1, -0.3, 0.0, -0.3]), np.array([1.0, 1.0, *([0.3] * 3)]), dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        u, v = action[:2]
        u = ((u + 1.0) * 0.5) * self.image_size[0]
        v = ((v + 1.0) * 0.5) * self.image_size[1]
